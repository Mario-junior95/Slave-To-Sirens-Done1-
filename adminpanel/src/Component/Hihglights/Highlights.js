...
function Highlights() {
    ...
    const addItem = async (e) => {
        e.preventDefault();
        try{
            await Axios.post('http://localhost:3001/admin/highlights/insert', {
                text : text,
                date : date,
                url : url,
                country : country
    ...
    const deleteItem = async (id) => {
        try{
            await Axios.delete(`http://localhost:3001/admin/highlights/delete/${id}`)
            .then((data) => {
                if(data.status === 200){
                    console.log(data)
                }
    ...
    const updateItem = async(id) => {
        Axios.put(`http://localhost:3001/admin/highlights/update/${id}`, {
            id : id ,
            text : newText , 
            url : newUrl,
            date : newDate,
    ...
    return (
        <div className = "container">
            ...
            <form>
                <h4>Add highlight Title : </h4>
               <textarea className = "textarea1" rows = '10' cols ="80" placeholder="Enter Higlight title..." value ={text} onChange = {(e) => {setText(e.target.value)}}></textarea><br/>
               <button onClick = {addItem}>Add Higlight</button>
               </form>
            </div>
            <div className = "Creat">
            ...
            <form>
                <h4>Update highlight Title : </h4>
               <textarea className = "textarea1" rows = '10' cols ="80" placeholder="Enter Higlight title..." onChange = {(e) => {setNewText(e.target.value)}}></textarea><br/>
               <button onClick={() => {updateItem(val._id)}}>Update Higlight</button>
               </form>
            </div> 
                    );
                })}
